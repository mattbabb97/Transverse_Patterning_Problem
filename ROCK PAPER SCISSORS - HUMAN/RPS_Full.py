...
class Trial(object):
    ...
    def newBlock(self):
        ...
        if self.AB_correct_pct > 0.89 and self.BC_correct_pct > 0.89 and self.CA_correct_pct > 0.89:
            self.consecutive += 1
...
data_file = makeFileName('Full_Transv_Patt')
...
while running:
    ...
    screen.fill(white)
    cursor.draw(screen)
    score_text = font.render(f'Score: {trial.score}', True, (0, 0, 0))
