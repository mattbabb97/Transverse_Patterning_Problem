...
# Phase 2: Insert the Delay between the Target and the Match-to-Sample
    ...
    def write(self, file, side, time_taken, correct):
        ...
        data = [monkey, today, now, self.step_wise_phase, self.block, self.trial_number, self.trial_type[self.trial_within_block], time_taken, side, correct]
...
data_file = makeFileName('Transverse_Patterning')
writeLn(data_file, ['monkey', 'date', 'time', 'stepwise_phase', 'block', 'trial_number', 'trial_type',
                    'response_time', 'response_side', 'correct_or_incorrect'])
